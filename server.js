...
async function initDB() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      api_key TEXT PRIMARY KEY, name TEXT, email TEXT, password TEXT,
      trial_end TIMESTAMP, key_limit INTEGER DEFAULT 10,
      is_admin BOOLEAN DEFAULT false, created_at TIMESTAMP DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS scripts (
      id SERIAL PRIMARY KEY, user_api_key TEXT REFERENCES users(api_key),
      name TEXT, original_code TEXT, obfuscated_code TEXT, created_at TIMESTAMP DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS license_keys (
      id TEXT PRIMARY KEY, user_api_key TEXT REFERENCES users(api_key),
      script_id INTEGER REFERENCES scripts(id), key TEXT, url_id TEXT UNIQUE,
      active BOOLEAN DEFAULT true, created_at TIMESTAMP DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS executions (
      id TEXT PRIMARY KEY, key_id TEXT REFERENCES license_keys(id),
      user_api_key TEXT, script_id INTEGER, hwid TEXT, timestamp TIMESTAMP DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS blacklist (
      id TEXT PRIMARY KEY, user_api_key TEXT REFERENCES users(api_key),
      target TEXT, reason TEXT, created_at TIMESTAMP DEFAULT NOW()
    );
    CREATE TABLE IF NOT EXISTS support (
      id SERIAL PRIMARY KEY, user_api_key TEXT, subject TEXT, message TEXT,
      reply TEXT, created_at TIMESTAMP DEFAULT NOW()
    );
  `);
...
app.post('/api/scripts', async (req, res) => {
  var { apiKey, name, originalCode, obfuscatedCode } = req.body;
  try {
...
app.get('/api/scripts', async (req, res) => {
  try {
    var result = await pool.query('SELECT * FROM scripts WHERE user_api_key = $1 ORDER BY created_at DESC', [req.query.apiKey]);
...
app.delete('/api/scripts/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM scripts WHERE id=$1 AND user_api_key=$2', [req.params.id, req.query.apiKey]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/keys', async (req, res) => {
  try {
    var result = await pool.query('SELECT * FROM license_keys WHERE user_api_key = $1 ORDER BY created_at DESC', [req.query.apiKey]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/keys/:id/toggle', async (req, res) => {
  try {
    await pool.query('UPDATE license_keys SET active = NOT active WHERE id = $1', [req.params.id]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/blacklist', async (req, res) => {
  var { apiKey, target, reason } = req.body;
  try {
    await pool.query(
...
app.get('/api/blacklist', async (req, res) => {
  try {
    var result = await pool.query('SELECT * FROM blacklist WHERE user_api_key = $1', [req.query.apiKey]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/support', async (req, res) => {
  var { apiKey, subject, message } = req.body;
  try {
...
app.get('/api/support', async (req, res) => {
  try {
    var result = await pool.query('SELECT * FROM support WHERE user_api_key = $1 ORDER BY created_at DESC', [req.query.apiKey]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/admin/users/:userKey', async (req, res) => {
  var { apiKey, days } = req.body;
  try {
    var admin = await pool.query('SELECT is_admin FROM users WHERE api_key = $1', [apiKey]);
    if (!admin.rows[0] || !admin.rows[0].is_admin) return res.status(403).json({ error: 'Admin only' });
    await pool.query("UPDATE users SET trial_end = NOW() + INTERVAL '" + days + " days' WHERE api_key = $1", [req.params.userKey]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete('/api/admin/users/:userKey', async (req, res) => {
  var apiKey = req.query.apiKey, userKey = req.params.userKey;
  try {
    var admin = await pool.query('SELECT is_admin FROM users WHERE api_key = $1', [apiKey]);
    if (!admin.rows[0] || !admin.rows[0].is_admin) return res.status(403).json({ error: 'Admin only' });
...
app.post('/api/admin/reset', async (req, res) => {
  try {
    var admin = await pool.query('SELECT is_admin FROM users WHERE api_key = $1', [req.body.apiKey]);
    if (!admin.rows[0] || !admin.rows[0].is_admin) return res.status(403).json({ error: 'Admin only' });
...
app.get('/api/admin/support', async (req, res) => {
  try {
    var admin = await pool.query('SELECT is_admin FROM users WHERE api_key = $1', [req.query.apiKey]);
    if (!admin.rows[0] || !admin.rows[0].is_admin) return res.status(403).json({ error: 'Admin only' });
